...
#Economic of Gas and Electric Boilers? Annualised Cost of Heat (£/yr) for each gas boilers
...
# Cost to goverment for switching from gas to electric boiler.
#
def make_c(i):
    def c(x, p):
        # Market share 
...
def c(x, p):
    ...
    #boiler = boiler_names[i]
    CTaxg = [x[boiler + ": Carbon Tax (£/yr)"] * val * p["Natural Gas Emission Factor (t CO2e/MWh)"] for val in (p[boiler_names[j] + ": Gas Consumption (MWh/year)"] for j in range(boilers))] 
    ...
    CTaxe = [x[boiler + ": Carbon Tax (£/yr)"] * val for val in (p[boiler_names[j] + ": Gas Consumption (MWh/year)"] * p["Electric Emission Factor (t CO2e/MWh)"] * p["Electric Boiler Efficiency"] for j in range(boilers))]

    FiTe = [p[boiler_names[j] + ": Gas Consumption (MWh/year)"] * p["Natural Gas Emission Factor (t CO2e/MWh)"] * x[boiler + ": Carbon Tax (£/yr)"] * p["Electric Boiler Efficiency"] for j in range(boilers)]

    Grant = [p["Electric Boiler Grant (of CAPEX)"] * ice for ice in ICe]

    Taxg = [vomg * (1 + x[boiler + ": Gas Tax (£/yr)"]) - vomg for vomg in VOMg]

    B = x[boiler + ": Market Share"] * CTaxe

    Costg = B - x[boiler + ": Market Share"] * CTaxe - (1-x[boiler + ": Market Share"]) * CTaxg - Taxg * (1-x[boiler + ": Market Share"])
    Costi = x[boiler + ": Market Share"] * CTaxe + CTaxg * (ACHe - ACHg - FiTe - Grant) + (1-x[boiler + ": Market Share"]) * Taxg + (1-x[boiler + ": Market Share"])* CTaxg

    return x['t'] - (Costg + Costi)
...
print(res)
df = pd.DataFrame(res)
#df.to_excel(r'C:\Users\Devan\OneDrive - Imperial College London\MSc\export_dataframe.xlsx', index=False)